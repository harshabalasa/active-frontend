...
const CssTextField = styled(TextField)({
  ...
  if(flag==0)
  obj.status=1;
  axios.post("http://localhost:777/api/tasks/update",obj).then((res)=>{
    if(res.status==200)
    {
      alert("Task updated succesfully");
      ReactDOM.render(<Router><AdminHome /></Router> ,document.getElementById("Admins_home_div"));
      ReactDOM.render(<Router><AdminHome /></Router>, document.getElementById("Admins_home_div"));
      ReactDOM.render(<Router><AdminHome /></Router>, document.getElementById("Admins_home_div"));
      ReactDOM.render(<Router><AdminHome /></Router> ,document.getElementById("Admins_home_div"));
    }
            ...
            <Box sx={{ margin: 1 }}>
              ...
              <Table size="small" aria-label="purchases">
                ...
                <TableBody>
                  ...
                  <TableRow><TableCell><Button style={{backgroundColor:'green',borderRadius:'2px',color:'black'}} onClick={()=>{
                    let newTask={
                    _id: `${props.task._id}`,
                    id1: `${props.task.id1}`,
                    id2: `${props.task.id2}`,
...
export default function AdminHome()
{
    ...
    useEffect(()=>{
      axios.get("https://active-backend.onrender.com/api/commissions").then((res)=>{
        ...
        filtera=res.data.commissions[0];
        }
        axios.post("https://active-backend.onrender.com/api/tasks/pendingList",{id:`${(filtera)?(filtera._id):(res.data.commissions[0]._id)}`}).then((res)=>{
    ...
    return(
      ...
      <DropdownButton className='select' id="dropdown-basic-button" title="commission name">
      {
        commisions.map((commission)=>{
          return (<Dropdown.Item key={commission._id} onClick={(e)=>{filtera=commission;
          axios.post("https://active-backend.onrender.com/api/tasks/pendingList",{id:`${commission._id}`}).then((res)=>{
        setTasks(res.data.tasks);
        })
